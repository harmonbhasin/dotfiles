# Refactoring UI: Design Systems & Values

## Spacing & Sizing Scale
Do not use a linear scale. Small jumps matter at small sizes; large jumps are required at large sizes to be noticeable. Use this hand-crafted geometric scale for margin, padding, width, and height.

| Variable | Value | Relative Increase |
| :--- | :--- | :--- |
| 0 | 0px | - |
| 1 | 4px | - |
| 2 | 8px | 100% |
...
## Typography Scale
Avoid em-based modular scales which result in fractional pixels. Use this pre-defined pixel scale.

*   12px, 14px, 16px (Base), 18px, 20px, 24px, 30px, 36px, 48px, 60px, 72px.

## Shadow System (Elevation)
Use two-part shadows for realism:
1.  **Ambient Occlusion:** Tight, dark, low blur. Represents the lack of light underneath the object.
2.  **Direct Light:** Large, soft, high blur. Represents the shadow cast by the light source.

**CSS Snippets:**
*   **Small (Button):** `0 1px 3px hsla(0,0%,0%,.2)`
*   **Medium (Dropdown):** `0 4px 6px hsla(0,0%,0%,.1), 0 1px 3px hsla(0,0%,0%,.1)`
*   **Large (Modal):** `0 15px 35px hsla(0,0%,0%,.2), 0 5px 15px hsla(0,0%,0%,.1)`

## Color Algorithms (HSL)

**1. Generating Shades:**
Do not just adjust lightness. To keep colors vibrant:
*   **To Lighten:** Increase Lightness AND rotate Hue toward the nearest bright hue (Yellow/Cyan/Magenta). Decrease Saturation slightly if near white.
*   **To Darken:** Decrease Lightness AND rotate Hue toward the nearest dark hue (Blue/Red/Green). Increase Saturation to prevent muddiness.

**2. Colored Greys:**
Never use `hsl(0, 0%, X%)`. Saturate greys to match "temperature".
*   **Cool Grey:** Add blue (Hue ~220, Saturation 5-15%).
*   **Warm Grey:** Add yellow/orange (Hue ~40, Saturation 5-15%).

**3. Text Contrast:**
*   **Dark background:** Rotate text hue toward Cyan/Yellow to increase perceived brightness without using pure white.
Step 3: Create the Main Skill File (SKILL.md)
Create ~/.claude/skills/refactor-ui/SKILL.md. This file explains the why and instructs Claude on how to apply the data from systems.md.
---
name: refactor-ui
